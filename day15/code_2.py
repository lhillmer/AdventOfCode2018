...
def try_attack(board, goblins, elves, atk_power):
    ...
    if not all([e.hp > 0 for e in elves]):
        print('Not all elves are alive :(')
    
    print('Trying attack power: {}'.format(atk_power))
    ...
    while True:
        ...
        for u in ordered_units:
            ...
            if a is not None and a.is_elf and a.hp <= 0:
                print('Failing on turn: {}'.format(turn_counter))
                return None
    ...
    return (turn_counter, winner_hp)

result = None
atk = 3
while result is None:
    atk += 1
    result = try_attack(board, goblins, elves, atk)

print('After {} turns, elves suffered no casualities with {} hp and {} atk. Result {}'.format(result[0], result[1], atk, result[0] * result[1]))
